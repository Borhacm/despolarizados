...
1. En el **SQL Editor** de Supabase, ejecuta **en este orden** (si solo ejecutas el seed, fallará: *relation medios does not exist*):
   ...
   - **Después** todo el contenido de `supabase/seed_medios.sql` (inserta los 10 medios).
...
3. Despliega. El archivo `vercel.json` define un cron hacia `/api/cron/ingest` cada 20 minutos; en el plan gratuito de Vercel el cron puede tener limitaciones: comprueba la [documentación de Vercel Cron](https://vercel.com/docs/cron-jobs).
...
- Clave OpenAI para `text-embedding-3-small` (1536 dimensiones)
